...
import { MapPin, Calendar, Eye, Search, Filter, X, Plus, MessageCircle, Bell, BellOff } from 'lucide-react';
...
const STATUS_CONFIG = {
  pendiente: { label: 'Pendiente', color: 'bg-yellow-100 text-yellow-800 border-0' },
  'en-proceso': { label: 'En Proceso', color: 'bg-blue-100 text-blue-800 border-0' },
  resuelto: { label: 'Resuelto', color: 'bg-green-100 text-green-800 border-0' },
  rechazado: { label: 'Rechazado', color: 'bg-red-100 text-red-800 border-0' },
};
...
export function PublicReportsTracking({ currentUser }: PublicReportsTrackingProps) {
  ...
  const handleToggleFollow = (report: Report) => {
    if (!currentUser) return;
    
    const isFollowing = isReportFollowed(report.id, currentUser.id);
    
    if (isFollowing) {
      const success = unfollowReport(report.id, currentUser.id);
      if (success) {
      ...
      const success = followReport(report.id, report.title, report.status, currentUser.id, commentCount);
      if (success) {
        toast.success('Ahora sigues este reporte', {
          description: 'Te notificaremos cuando cambie de estado o reciba comentarios'
        });
        setFollowedReportsCount(getFollowedReportsCount(currentUser.id));
        setUnreadNotificationsCount(getUnreadNotificationsCount(currentUser.id));
      }
  ...
  useEffect(() => {
    ...
    const checkChanges = async () => {
      ...
      // Verificar actualizaciones y mostrar toasts cuando está activo
      ...
      notifications.forEach(notification => {
        if (notification.type === 'status_change') {
          toast.info(`📢 Actualización: ${notification.reportTitle}`, {
            description: `Estado cambió de "${notification.oldStatus}" a "${notification.newStatus}"`,
            duration: 6000,
          });
        } else if (notification.type === 'new_comment') {
          toast.info(`💬 Nuevo comentario: ${notification.reportTitle}`, {
            description: `${notification.newCommentsCount} nuevo${notification.newCommentsCount! > 1 ? 's' : ''} comentario${notification.newCommentsCount! > 1 ? 's' : ''}`,
            duration: 6000,
          });
        }
      });
                        ...
                        <div className="flex items-center justify-between flex-wrap gap-2">
                          ...
                          <div className="flex gap-2 flex-wrap">
                            ...
                            <Button
                              size="sm"
                              variant="outline"
                              className={`${STATUS_CONFIG[report.status].color} border-2 px-3 py-2 h-9 cursor-default pointer-events-none`}
                            >
                            ...
                            {currentUser && (
                              <Button
                                ...
                                variant="outline"
                                className="border-green-200 hover:bg-green-50"
                                onClick={() => handleToggleFollow(report)}
                              >
