...
export function PublicReportsTracking({ currentUser }: PublicReportsTrackingProps) {
  ...
  const handleToggleFollow = (report: Report) => {
    ...
    if (isFollowing) {
      ...
      const success = followReport(report.id, report.title, report.status, currentUser.id);
      if (success) {
        toast.success('Ahora sigues este reporte', {
          description: 'Te notificaremos cuando cambie de estado'
        });
        setFollowedReportsCount(getFollowedReportsCount(currentUser.id));
      }
    }
  ...
  useEffect(() => {
    ...
    const checkChanges = () => {
      const notifications = checkForStatusChanges(reports, currentUser.id);
    ...
    const interval = setInterval(checkChanges, 60000); // Check every minute
